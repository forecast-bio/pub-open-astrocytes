...
#### Running this pub
If you're running through the examples in this pub yourself, you'll need additional scripts in separate modules from the snippets below with some bonus content; the full reproducible source for this pub can be found in [this repo](https://github.com/forecast-bio/pub-open-astrocytes/). You'll find the code for generating all the figures folded, like the below:
...
first_movie_frames = get_movie( FIRST_MOVIE_ID, verbose = False )
first_movie_times, movie_dt = movie_times( first_movie_frames )

# TODO The current `OpenAstrocytes` data architecture has imaging channels in
# the first axis; we're refactoring the frame schemas to make channel contents
    ...
    for i_patch in range( n_patches ):
        ...
        for i_t in range( n_frame_compare ):
            ...
            if i_patch == 0:
                ...
                ax.set_title( f'{first_movie_times[i_t]:+0.1f}s',
                    fontsize = 12,
                )
...
"TODO Migrate over the PCA results to the full typed schema in `astrocytes`"
...
with HouseStyle( grids = True ) as s:
    ...
    for i_patch in range( n_patches ):
        ...
        for i_seq, (cur_pc, cur_color) in enumerate(
                    zip( DEMO_PCS, DEMO_PC_COLORS )
                ):
...
GRIDS_YTICKS = [-10, 0, 10]


##

# Collate full dataset
first_movie_traces = get_movie_traces(
    FIRST_MOVIE_ID,
    verbose = DEV,
)
...
with HouseStyle() as s:
    ...
    for i in range( N_PATCHES_Y ):
        for j in range( N_PATCHES_X ):
            ...
            cur_values_norm, _, _ = baseline_normalize(
                t_rel, trace.values[plot_pc, :],
                BASELINE_WINDOW,
            )  
            ...
            ax.fill_between( [0, xl[1]], GRIDS_YLIM[0], GRIDS_YLIM[1],
                color = 'k',
                alpha = 0.05,
                linewidth = 0,
                zorder = 100,
            ...
            if (i, j) in DEMO_PATCHES:
                ax.fill_between( [xl[0], xl[1]], GRIDS_YLIM[0], GRIDS_YLIM[1],
                    facecolor = 'none',
                    edgecolor = f'k',
                    alpha = 0.7,
                    linewidth = 2.,
    ...
    for i in range( 1, N_PATCHES_X ):
        axs[-1, i].set_xticks( GRIDS_XTICKS, ['' for _ in GRIDS_XTICKS] )
    
    axs[-1, 0].set_xticks( GRIDS_XTICKS, [f'{x:+0.0f}' for x in GRIDS_XTICKS],
        fontsize = 11,
        rotation = 90,
    )

    axs[-1, 0].set_yticks( GRIDS_YTICKS, [f'{x:+0.0f}' for x in GRIDS_YTICKS],
        fontsize = 11,
    )
...
with HouseStyle() as s:
    ...
    for i in range( N_PATCHES_Y ):
        for j in range( N_PATCHES_X ):
            ...
            cur_values_norm, _, _ = \
                baseline_normalize( t_rel, trace.values[i_pc, :], baseline_window )  
